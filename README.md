...
La soluzione analitica del problema è nota, e per semplificare i calcoli
considereremo un potenziale nullo. In questo caso, gli autovalori sono
dati da:
$$E_{n_x,n_y} = 2\pi^2 \left( \frac{n_x^2}{L_x^2} + \frac{n_y^2}{L_y^2} \right)$$
dove $n_x, n_y$ sono i numeri quantici associati alle dimensioni $x$ e
$y$. Otteniamo così che i primi autovalori sono: $$\begin{aligned}
    E_{0,0} &= 0 \\ E_{1,0} &= 0.012 \\ E_{0,1} &= 0.012 \\ E_{1,1} &= 0.024 \\
    E_{2,0} &= 0.048\end{aligned}$$ Nel caso dell'utilizzo di unità
naturali. I valori invece ricavati dal nostro algoritmo sono:
`Eigenvlaues: 0.0 0.012 0.012 0.012 0.012 0.024 0.024 0.024 0.024 0.048`\
Possiamo attribuire la discrepanza in molteplicità degli autovalori al
fatto che traslazioni o rotazioni della funzione d'onda non ne
modifichino l'energia, ma essendo ortogonali tra loro, non sono
considerati come autostati distinti, anche se fisicamente equivalenti.
